...
/**
 * Created by 506605454 on 2015/12/3 0003.
 */
...
public class MyViewPager extends ViewPager {
        ...
        switch (arg0.getAction()) {
            case MotionEvent.ACTION_UP:
                ...
                if ((currentItem == 0 || currentItem == pagerCount - 1)){
                    float nowX0 = arg0.getX();
                    float offset0 = nowX0 - preDwon;
                    if(currentItem == 0){
                        if (offset0>0){
                           IgetPageListenter.getNewPage();
                        }
                    }else {
                        if (offset0<0){
                            IgetPageListenter.getOldPage();
                        }
                    }
                }
    ...
    public interface IGetPageListenter{
       public void getNewPage();
       public void getOldPage();
   }

    IGetPageListenter IgetPageListenter = null;
    public void setGetPageListenter(IGetPageListenter getPageListenter){
        this.IgetPageListenter =getPageListenter;
